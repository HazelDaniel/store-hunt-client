				...
				<section className="checkout-form-container">
					...
					<form>
						<div className="inline-input-div">
							...
							<CustomInput type={"text"} label={`last name`} id={`last-name`} placeholder={`smith`} $variant={`S`} />
						</div>
						<CustomInput type={"email"} label={`email`} id={`email`} placeholder={`example@example.com`} $variant={`L`} />
						<div className="inline-input-div">
							<CustomInput type={"tel"} label={`phone`} id={`phone`} placeholder={`+ XXX-XXX-XXX`} $variant={`S`} />
						</div>
						...
						<CustomInput type={"password"} label={`password`} id={`password`} placeholder={`password`} $variant={`L`} />
						<CustomInput type={"password"} label={`confirm password`} id={`confirm-password`} placeholder={`confirm password`} $variant={`L`} />
					...
					<section className="confirmation-section">
						...
						<p className="confirmation-text">
							Let's not forget that a broad understanding of the essence will require a vast knowledge. The some of the individual elements gives less satisfactory results. At the same
							time, however, a lot of effort has been invested into the crucial component. By the way, the initial progress in the participant evaluation sample partially the competitive
						...
						<form className="confirmation-form">
							<div className="agreement-check-div">
								...
								<label for="agreement-checkbox">
									<span>I Agree and accept to be held accountable for deductions on my account using this service</span>
								</label>
							</div>
